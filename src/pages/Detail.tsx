    ...
    return (
        <Container>
            <Grid container spacing={3}>
                ...
                <Grid item md={7}>
                    <Stack sx={{py: 3}} direction={"row"} justifyContent={"space-between"} alignItems={"center"}>
                        <Breadcrumbs separator={<NavigateNextIcon fontSize="small"/>}>
                            <NavLinkMUILink component={NavLink} top={"/"} underline="hover">
                                <Typography variant={"subtitle1"}> Trang chủ</Typography>
                            </NavLinkMUILink>
                            <NavLinkMUILink component={NavLink} top={"/"} underline="hover">
                                <Typography variant={"subtitle1"}>{toCategoryName(detail.category)}</Typography>
                            </NavLinkMUILink>
                        </Breadcrumbs>
                    ...
                    <Typography gutterBottom component={"h1"} variant={"h2"}>{detail.title}</Typography>
...
function NewsImage({image, showImage}: { image: Image | undefined, showImage: () => void }) {
    if (typeof image == "undefined") return;

    return (
        <Box>
            <CardMedia component={"img"} image={image.link} onClick={showImage}/>
            <Typography variant={"body2"} gutterBottom paragraph={true} align={"center"}>{image.capture}</Typography>
        </Box>
    )
}

function NewParagraph({paragraph, showImage}: { paragraph: Paragraph, showImage: () => void }) {
    return (
        <>
            <Typography gutterBottom variant={"body1"} align={"justify"}>{paragraph.text}</Typography>
            <NewsImage key={Math.random()} image={paragraph.image} showImage={showImage}/>
        </>
    );
}
