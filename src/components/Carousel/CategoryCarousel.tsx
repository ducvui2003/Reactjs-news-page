    ...
    return (
        <>
            <Divider
                ...
                sx={{height: '1.5px', backgroundColor: 'black', opacity: 0.2, marginBlock: '10px'}}
            />
            <Carousel title={category}>
                {news.slice(0, quantity).map((item: NewsLoading): any => {
                    return <SwiperSlide style={{height: "100%"}}><CardHorizontal news={item.news}
                                                                                 isLoading={loading}/></SwiperSlide>
                })}
            </Carousel>
        </>
    );
